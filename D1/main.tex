...
% Begin SubSection
\begin{itemize}
	This SRS focuses on functional requirements, non-functional requirements, user interactions, and security protocols for the SecureChat application. This document is written for the intention of usage by a broad audience including project managers, software developers, security analysts, and stakeholders. No prerequisite knowledge or readings is expected or required.
\end{itemize}
% End SubSection
...
% Begin SubSection
\begin{itemize}
	The SecureChat application will be an encrypted messaging app intended for deployment on company-issued Android devices. It will integrate a Key Distribution Centre (KDC) to manage cryptographic keys, an authentication protocol to validate user identities, and will also employ symmetric-key encryption to ensure secure communications within the app. SecureChat will facilitate secure text messaging, file sharing, and guarantee the integrity and confidentiality of chat logs. In addition to these, some other functionalities SecureChat will have include dynamic key generation and rotation, as well as ensuring resilience against potential security threats.

One objective of SecureChat is that it be able to establish a secure, efficient channel of communication within an organization. And therefore, through this, mitigate any risks associated with corporate espionage. It will be utilizing advanced cryptographic techniques to achieve this. SecureChat’s aim will be to protect sensitive information while maintaining user-friendly operations. SecureChat also seeks to foster trust among employees in the organization's commitment to security and privacy.
